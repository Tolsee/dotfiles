...
- Delegate all actual code edits (Edit/Write/file changes) to a subagent — never write or edit code directly from the main session. The main session handles research, planning, review, and orchestration; subagents make the changes. **This rule applies only to the top-level session.** A dispatched subagent must make its assigned edits itself and must never spawn further subagents to do so — if it did, this same rule read from global context would otherwise recurse indefinitely.
- **Default to the mid-tier model for implementation.** The top-tier main session does the *logical* work — diagnosis, deciding the exact change, verifying the result — and hands the *implementation* down. A top-tier subagent re-derives reasoning the main session already did, so it costs far more for the same diff. Tiers: mechanical/well-specified edits, renames, an already-diagnosed one-liner → cheapest model; everything else, including multi-file feature work, bug fixes and refactors → mid-tier. Reserve the top tier for work where the *structure itself* is still undecided and getting it wrong is expensive to unwind. "This spans several files" or "this looks intricate" is not a reason. If the main session has already spelled out the change, always mid-tier.
